...
public class BusType {
        ...
        consumerProps.put(ConsumerConfig.GROUP_ID_CONFIG, "BUSTYPE");
                ...
                for (ConsumerRecord<String, JsonNode> record : records) {
                    ...
                    * CREATE TABLE BusType
                    (
                        ...
                        MNTN_TYP_NM VARCHAR(20) unique,
                        MNTN_TYP_CD VARCHAR(20)
                    );
                    */           
                    
                    // PreparedStatement 객체 생성
                    PreparedStatement pstmt = conn.prepareStatement("insert ignore into BusType set MNTN_TYP_NM = ?, MNTN_TYP_CD = ?");

                    // 파라미터 바인딩
                    String bss_ars_no = record.value ().get ("payload").get ("after").get ("MNTN_TYP_NM").toString();
                    String bus_sta_nm = record.value ().get ("payload").get ("after").get ("MNTN_TYP_CD").toString();
                    System.out.print("mntn_typ_nm : "+bss_ars_no);
